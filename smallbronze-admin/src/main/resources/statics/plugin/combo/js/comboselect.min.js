;(function ($) {
    "use strict";
    var constants = {
        dataKey: 'comboSelectObject',
        objStatusKey: 'comboSelect-self-mark',
        objStatusIndex: 'comboSelect-self-index'
    };
    ...
    $.extend(ComboSelect.prototype, {
        _setOption: function (source, option) {
            ...
            if ($(combo_input).hasClass('input-block-level')) $(elem.container).css('width', '100%'); else $(elem.container).width($(combo_input).outerWidth());
            ...
            var hidden_name = input_name, hidden_id = input_id;
            if (input_name.match(/\]$/)) input_name = input_name.replace(/\]?$/, namePrefix); else input_name += namePrefix;
            if (input_id.match(/\]$/)) input_id = input_id.replace(/\]?$/, namePrefix); else input_id += namePrefix;
            elem.hidden = $('<input type="hidden" class="cs_hidden" />').attr({
                name: hidden_name,
                id: hidden_id
            }).val('');
            $(elem.combo_input).attr({name: input_name, id: input_id});
            ...
            $(document).off('mousedown.comboSelect').on('mousedown.comboSelect', function (e) {
                if ($(document.body).data(constants.objStatusKey)) $(document.body).data(constants.objStatusKey, false); else {
                    ...
                    $('div.cs_container').each(function () {
                        ...
                        if ($(this).hasClass(d.css_class.container_open)) {
                            ...
                            if ($('li', $(d.elem.results)).length > 0) {
                                if (d.option.auto_fill_result) {
                                    if ($('li.cs_selected', $(d.elem.results)).length > 0) {
                                        d._hideResults(d);
                                    } else if ($('li.cs_over', $(d.elem.results)).length > 0) {
                                        if ($(d.elem.hidden).val()) d._hideResults(d); else d._selectCurrentLine(d, true);
                                    } else if (d.option.auto_select_first) {
                                        if ($(d.elem.hidden).val()) d._hideResults(d); else {
                                            d._nextLine(d);
                                ...
                                if (d.option.no_result_clean) cleanContent(d); else {
            ...
            if (($.inArray(e.keyCode, [27, 38, 40, 9]) > -1 && $(self.elem.result_area).is(':visible')) || ($.inArray(e.keyCode, [37, 39, 13, 9]) > -1 && self._getCurrentLine(self)) || (e.keyCode == 40)) {
                ...
                switch (e.keyCode) {
                    case 37:
                        if (e.shiftKey) self._firstPage(self); else self._prevPage(self);
                        break;
                    case 38:
                        ...
                        break;
                    case 39:
                        if (e.shiftKey) self._lastPage(self); else self._nextPage(self);
                        break;
                    case 40:
                ...
                if (e.keyCode != 16) self._setCssFocusedInput(self);
                self._inputResize(self);
            ...
            if (typeof self.option.source == 'object') self._searchForJson(self, q_word, which_page_num); else self._searchForDb(self, q_word, which_page_num);
            ...
            for (i = 0; i < self.option.source.length; i++) {
                ...
                for (var j = 0; j < arr_reg.length; j++) {
                    var itemText = row[self.option.field];
                    if (self.option.format_item && $.isFunction(self.option.format_item)) itemText = self.option.format_item(row);
            ...
            if (self.prop.is_paging === false) self._setCssFocusedInput(self); else {
                if (!self.option.multiple) {
            ...
            var buildPageNav = function (self, pagebar, page_num, last_page) {
                if ($('li', $(pagebar)).length == 0) {
                    ...
                    if (page_num == last_page) btnclass = ' disabled '; else btnclass = '';
            ...
            if (last_page > 1) {
                if (self.prop.is_suggest) self.prop.max_suggest = last_page; else self.prop.max_all = last_page;
            ...
            for (var i = 0; i < arr_candidate.length; i++) {
                ...
                var list = $('<li>').html(itemText).attr({pkey: arr_primary_key[i], title: itemText});
            ...
            $(self.elem.result_area).show(1, function () {
                if ($(self.elem.container).css('position') == 'static') {
                    ...
                    $(self.elem.result_area).css({left: left + 'px'});//top: top + 'px',
                }
            });
            ...
            if ($(obj).length)return obj; else return false;
            ...
            if ($('li.selected_tag', $(self.elem.element_box)).length == 0) {
